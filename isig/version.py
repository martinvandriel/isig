...
#
# Modified by the ObsPy project, instaseis and pymesher.
...
# NO IMPORTS FROM PYMESHER OR FUTURE IN THIS FILE! (file gets used at
# installation time)
import io
import os
import inspect
...
PYMESHER_ROOT = os.path.abspath(os.path.join(script_dir, os.pardir))
...
def call_git_describe(abbrev=4):
    try:
        p = Popen(['git', 'rev-parse', '--show-toplevel'],
                  cwd=PYMESHER_ROOT, stdout=PIPE, stderr=PIPE)
        p.stderr.close()
        path = p.stdout.readline().decode().strip()
        p.stdout.close()
    except:
        return None
    if os.path.normpath(path) != PYMESHER_ROOT:
        return None
    try:
